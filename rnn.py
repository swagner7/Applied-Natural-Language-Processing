...
class RNN(nn.Module):
    def __init__(self, vocab, num_classes):
        ...
        self.rnn = nn.RNN(self.embed_len, self.hidden_dim, num_layers=self.n_layers, bidirectional=True, batch_first=True)
    ...
    def forward(self, inputs, inputs_len):
        ...
        rnn_output = self.forward_rnn(embedded, inputs_len)
    ...
    def forward_concat(self, rnn_output, inputs_len):
        ...
        # Find the last non-zero index in each sequence
        last_non_zero_idx = inputs_len - 1
        forward_hidden = rnn_output[range(rnn_output.shape[0]), last_non_zero_idx, :self.hidden_dim]
        backward_hidden = rnn_output[range(rnn_output.shape[0]), 0, self.hidden_dim:]
