        ...
        for sentence in data:
            ...
            for word in words:
                try:
                    ...
                    # Word not found in the glove model, so ignore it
                    pass

            if len(embeddings) > 0:
                # Calculate the mean of word embeddings in the sentence
                mean_embedding = np.mean(embeddings, axis=0)
                transformed_features.append(mean_embedding)
            else:
                # If none of the words were found, use a zero vector
